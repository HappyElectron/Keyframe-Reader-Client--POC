...
namespace CesiumForUnity
{
    ...
    /// looks right-side up.
    /// 
    /// Modified for a mobile-specific experience. Works with Joystick image UI
    /// components to provide natural 3d movement.
    /// </summary>
    [RequireComponent(typeof(CesiumOriginShift))]
    [RequireComponent(typeof(Camera))]
    [DisallowMultipleComponent]
    [AddComponentMenu("Modified Camera Controller")]
    public class ModifiedCameraController : MonoBehaviour
    {
        ...
        private bool RightTouchAssigned;
        ...
        /* This code is distinctly not-so-good
         * 
         * COULD be using events rather than checking touch state every frame, 
         * but since this is only a POC, this is how the default cesium controller 
         * did mobile, and it seems to be working alright, I'm going to leave it 
         * for later if we need to 'boost' performance.
         */
        private void HandlePlayerInputs()
        {
            ...
            {
                // Move control; joystick only, panning with 2 fingers is dumb, IMHO.
                if (touch.startScreenPosition.x < Screen.width / 2)
                {
                    // If a left touch has not been assigned, or the last touch has ended
                    // Set a new touch, and show joystick
                    if(!LeftTouchAssigned || LeftTouch.phase == UnityEngine.InputSystem.TouchPhase.Ended)
                    {
                        LeftTouch = touch;
                        LeftTouchAssigned = true;

                        // Activate and position the joystick
                        LeftJoystick.SetActive(true);
                        LeftJoystick.transform.position = touch.screenPosition;
                    }

                    if (!handledMove && touch.startScreenPosition == LeftTouch.startScreenPosition)
                    {
                        handledMove = true;
                ...
                // Look controls; panning XOR joystick.
                else
                {
                    if (JoystickLook)
                    {
                        // If a left touch has not been assigned, or the last touch has ended
                        // Set a new touch, and show joystick
                        if (!RightTouchAssigned || RightTouch.phase == UnityEngine.InputSystem.TouchPhase.Ended)
                        {
                            RightTouch = touch;
                            RightTouchAssigned = true;

                            // Activate and position the joystick
                            RightJoystick.SetActive(true);
                            RightJoystick.transform.position = touch.screenPosition;
                        }

                        if (!handledLook && touch.startScreenPosition == RightTouch.startScreenPosition)
                        {
                            lookDelta = (touch.screenPosition - touch.startScreenPosition) / LookSpeedScaler;
                            handledLook = true;
                        }
                    }
                    // Rightside panning behaviour
                    else if (!handledLook)
                    {
            ...
            // If touch has left the screen, remove joysticks
            if (!LeftTouchAssigned || (LeftJoystick.activeInHierarchy && LeftTouch.phase == UnityEngine.InputSystem.TouchPhase.Ended))
            {
                LeftJoystick.SetActive(false);
                LeftTouchAssigned = false;
            }
            if (!RightTouchAssigned || (RightJoystick.activeInHierarchy && RightTouch.phase == UnityEngine.InputSystem.TouchPhase.Ended))
            {
                RightJoystick.SetActive(false);
                RightTouchAssigned = false;
            }
#endif
